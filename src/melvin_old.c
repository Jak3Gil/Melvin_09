...
static SparseEmbedding* node_ensure_sparse_embedding(Node *node);
...
/* Forward declarations for sparse context functions (still used in some places) */
...
/* Forward declarations for node weight functions */
static inline float node_get_local_outgoing_weight_avg(Node *node);

/* Passive context computation (no storage in edges, derived from current state) */
static float edge_compute_passive_context_signal(Edge *edge, ActivationPattern *wave_state);
static float edge_compute_context_weight(Edge *edge, SparseContext *current_context);

/* ============================================================================
 * EDGE TRANSFORMER (On-Demand Using Node Embeddings)
 * 
 * Implements Requirement.md line 7: "edges transform locally in the same LLM transform globally"
 * MEMORY-EFFICIENT: Uses node embeddings as Q, K, V on-demand (no per-edge storage)
 * - Query (Q) = from_node->sparse_embedding
 * - Key (K) = to_node->sparse_embedding
 * - Attention = Q·K similarity (sparse dot product, O(k))
 * - Zero memory overhead per edge
 * ============================================================================ */
 ...
 */
/* ============================================================================
 * NODE: Minimal Brain-Scale Structure (Requirement.md: < 20 bytes on disk)
 * ============================================================================
 * 
 * BRAIN-SCALE: Must support 86 billion nodes
 * - On-disk: ~20 bytes + payload (stored in .m file)
 * - Runtime: Additional computed/cached fields (RAM only when active)
 * 
 * Principle: COMPUTE ON DEMAND - don't store what can be computed
 * - Weight sums: computed from edges when needed
 * - Best edge: computed by scanning edges when needed
 * - Activation: computed from incoming edges when needed
 * - Embeddings: computed lazily, cached in runtime
 */
typedef struct Node {
    // === ESSENTIAL (stored on disk, ~20 bytes + payload) ===
    uint8_t id[9];                    // 9 bytes - Unique identifier
    uint8_t *payload;                 // 8 bytes ptr - Actual data (stored separately)
    size_t payload_size;              // 8 bytes - Size in bytes (0 = blank node)
    uint8_t port_id;                  // 1 byte - Port ID (multi-modal routing)
    uint8_t abstraction_level;        // 1 byte - Hierarchy level (0=raw, 1+=hierarchy)
    uint8_t is_reference_hierarchy;   // 1 byte - 1=references children, 0=copied payload
    uint8_t flags;                    // 1 byte - Packed flags (is_current_input, loaded, etc)
    
    // === RUNTIME ONLY (computed/cached, not stored on disk) ===
    // Edge arrays (loaded on-demand from edge index)
    struct Edge **outgoing_edges;     // Runtime: loaded from edge index
    size_t outgoing_count;            // Runtime: count of outgoing edges
    size_t outgoing_capacity;         // Runtime: capacity of edge array
    
    struct Edge **incoming_edges;     // Runtime: loaded from edge index
    size_t incoming_count;            // Runtime: count of incoming edges
    size_t incoming_capacity;         // Runtime: capacity of edge array
    
    // Computed on-demand (O(degree) when needed, cached in runtime)
    float activation_strength;        // Computed from incoming edges
    float cached_outgoing_sum;        // Computed: sum of outgoing edge weights
    float cached_incoming_sum;        // Computed: sum of incoming edge weights
    uint32_t cache_generation;        // Generation when caches were computed
    
    // Weight sums (maintained incrementally for O(1) access, runtime only)
    float outgoing_weight_sum;        // Runtime: cached sum of outgoing weights
    float incoming_weight_sum;        // Runtime: cached sum of incoming weights
    
    // Multi-functional variables (runtime, not stored)
    float weight;                     // Runtime: importance, age, usage frequency
    float bias;                       // Runtime: meta-learning rate, exploration
    float state;                      // Runtime: hidden state, momentum
    
    // Adaptive learning rate tracking (runtime only)
    float *recent_weight_changes;     // Runtime: rolling window of weight changes
    size_t weight_change_capacity;    // Runtime: capacity of changes array
    size_t weight_change_count;       // Runtime: count of recorded changes
    size_t weight_change_index;       // Runtime: current index in circular buffer
    float change_rate_avg;            // Runtime: averaged change rate
    
    // Self-destruct tracking (runtime only)
    float inactivity_timer;           // Runtime: increments when not activated
    uint32_t total_activations;       // Runtime: activation count
    float stop_weight;                // Runtime: learned weight for stopping
    
    // Context-relative edge values (runtime caching)
    float *edge_context_values;       // Runtime: cached edge values
    size_t edge_context_capacity;     // Runtime: capacity
    struct Edge *best_edge;           // Runtime: cached best edge
    float best_edge_value;            // Runtime: cached best value
    uint32_t context_generation;      // Runtime: generation for invalidation
    
    // Attention (runtime only)
    float *attention_keys;            // Runtime: what this node "looks for"
    float *attention_values;          // Runtime: what this node "provides"
    size_t attention_dim;             // Runtime: attention dimension
    
    // Predictive coding (runtime only)
    float predicted_activation;       // Runtime: prediction
    float prediction_error;           // Runtime: error
    
    // Flags packed (runtime)
    uint8_t is_current_input;         // Runtime: 1 = currently input
    uint8_t loaded;                   // Runtime: 1 = loaded from disk
    
    // MiniNet (loaded separately, can be shared or lazy-init)
    MiniNet *net;                     // Runtime: loaded on-demand
    
    // Hierarchy (child node IDs stored, pointers loaded on-demand)
    Node **child_nodes;               // Runtime: loaded from child IDs
    size_t child_count;               // Runtime: number of children
    
    // Lazy-computed embeddings (computed on first access)
    float *embedding;                 // Runtime: computed on-demand
    size_t embedding_dim;             // Runtime: embedding dimension
    uint32_t embedding_generation;    // Runtime: invalidation tracking
    
    // File/loading state
    off_t file_offset;                // Runtime: position in .m file
    struct Graph *graph;              // Runtime: back-reference to graph
    
    // Sparse embedding (computed on-demand)
    SparseEmbedding *sparse_embedding; // Runtime: computed when needed
    
    // === RUNTIME OPTIONAL FEATURES (NULL when not used, lazy-init) ===
    // These are runtime-only, computed on-demand, not stored on disk
    struct Node *abstraction_parent;        // Runtime: parent abstraction (NULL if none)
    struct Node **semantic_neighbors;       // Runtime: cached similar nodes (NULL = not computed)
    float *semantic_similarities;           // Runtime: similarity values
    size_t semantic_neighbor_count;         // Runtime: count of cached neighbors
    size_t semantic_neighbor_capacity;      // Runtime: capacity
    struct ModalityBridge *modality_bridge; // Runtime: cross-modal binding (NULL if single-modal)
    struct AttentionMechanism *attention;   // Runtime: attention state (NULL = not used)
} Node;
...
typedef struct Edge {
    ...
    // PASSIVE CONTEXT: Minimal storage, derived from computation
    // Context emerges from:
    //   1. Target node activation (wave propagation)
    //   2. Embedding similarity (semantic context)
    //   3. Mini-net learned preferences (experience)
    // Context tags kept for backward compatibility but NOT populated (tag_count = 0)
    ContextTag *context_tags;         // DEPRECATED: Not populated (always NULL)
    size_t tag_count;                 // DEPRECATED: Always 0 (passive context)
    size_t tag_capacity;              // DEPRECATED: Always 0
    ...
    // TRANSFORMER ATTENTION (on-demand using node embeddings)
    // Implements Requirement.md line 7: "transform locally in the same LLM transform globally"
    // Uses node embeddings as Q, K, V on-demand (no storage, zero memory overhead)
} Edge;
...
typedef struct Graph {
    ...
    // STOP NODE (Like LLM EOS Token)
    // Every node can have an edge to STOP, trained like any other edge
    // During generation, STOP edge competes with regular edges
    // If STOP edge wins, generation stops (like EOS token in LLMs)
    Node *stop_node;                  // Singleton STOP node (like EOS token)
...
/* ============================================================================
 * PASSIVE CONTEXT COMPUTATION (Brain-Like, Minimal Storage)
 * ============================================================================
 * 
 * Brain: Context emerges from current neural activity, not stored explicitly
 * - No per-edge "context storage" - just connection strengths and current activations
 * - Context = which neurons are currently active
 * - Decisions emerge from activation patterns flowing through connections
 * 
 * Melvin: Context derived from computation, not stored in edges
 * - activation_strength: Which nodes are active (wave propagation)
 * - sparse_embedding: Semantic similarity (learned from co-occurrence)
 * - edge weights: Connection strengths (Hebbian learning)
 * - mini_net: Learned decision functions (experience)
 * 
 * Requirement.md line 11: "context changes the edge weights of the current node"
 * - Context = current activation pattern
 * - Edge weights modulated by target activation (passive, minimal storage)
 * - SparseContext still used temporarily during computation, but NOT stored in edges
 * ============================================================================ */

/* SparseContext: Temporary structure for context computation (not stored in edges)
 * - Used during computation only
 * - NOT stored per-edge (that would violate Requirement.md line 32)
 * - Created on-demand, freed immediately after use
 */
typedef struct SparseContext {
    Node **active_nodes;      // Sparse array of active nodes
    float *activations;       // Activation strength per node
    uint8_t *port_ids;        // Port ID for each active node
    uint32_t *abstraction_levels;  // Abstraction level per node
    size_t count;             // Number of active nodes
    size_t capacity;          // Allocated capacity
    uint32_t generation;      // When this context was created
    uint32_t max_abstraction_level;  // Highest abstraction level
} SparseContext;

/* ContextTag: REMOVED from edges (was violating Requirement.md line 32)
 * - Edges no longer store context tags
 * - Context is now passive (computed from activation, not stored)
 */
typedef struct ContextTag {
    SparseContext *context;        // DEPRECATED: No longer used in edges
    float weight_contribution;     // DEPRECATED
    uint32_t creation_time;        // DEPRECATED
    uint32_t last_activation;      // DEPRECATED
} ContextTag;
...
/* SparseContext functions kept for temporary computation
 * NOT stored in edges - only used during computation, then freed
 * This is brain-like: context computed on-demand, not stored
 */
static SparseContext* sparse_context_create_from_nodes(Node **nodes, float *activations, size_t count) {
    ...
    for (size_t i = 0; i < count; i++) {
        if (nodes[i] && (activations == NULL || activations[i] > 0.0f)) {
            active_count++;
        }
    }
    ...
    for (size_t i = 0; i < count; i++) {
        if (nodes[i] && (activations == NULL || activations[i] > 0.0f)) {
            ...
            ctx->activations[idx] = activations ? activations[i] : 1.0f;
            ctx->port_ids[idx] = nodes[i]->port_id;
            ctx->abstraction_levels[idx] = nodes[i]->abstraction_level;
            if (nodes[i]->abstraction_level > max_level) {
                max_level = nodes[i]->abstraction_level;
            }
            idx++;
        }
    }
    ...
    ctx->generation = 0;
    ctx->max_abstraction_level = max_level;
...
/* DUPLICATE REMOVED: sparse_context functions already defined above */
...
/* Ensure node has sparse embedding (lazy initialization)
 * - Creates embedding on first access
 * - Initializes from node payload + context
 * - O(k) where k = payload_size + neighbor_count
 */
static SparseEmbedding* node_ensure_sparse_embedding(Node *node) {
    if (!node) return NULL;
    
    // If embedding exists and is valid, return it
    if (node->sparse_embedding && node->sparse_embedding->active_count > 0) {
        return node->sparse_embedding;
    }
    ...
    // Initialize from payload (hash-based dimensions)
    if (node->payload_size > 0) {
        for (size_t i = 0; i < node->payload_size && i < 16; i++) {
            // Hash each byte to a dimension (spread across embedding space)
            uint16_t dim = (uint16_t)((node->payload[i] * 17 + i * 257) % 65536);
            float value = (float)node->payload[i] / 255.0f;
            sparse_embedding_set_dimension(emb, dim, value);
        }
    }
    
    // Also include context from neighbors (if available)
    // This makes embeddings richer and more semantic
    if (node->incoming_count > 0 || node->outgoing_count > 0) {
        SparseEmbedding *context_sig = compute_context_signature(node);
        if (context_sig) {
            sparse_embedding_update_hebbian(emb, context_sig, 0.3f);
            sparse_embedding_free(context_sig);
        }
    }
    
    sparse_embedding_normalize(emb);
    node->sparse_embedding = emb;
    return emb;
}
...
/* ============================================================================
 * EDGE TRANSFORMER IMPLEMENTATION (On-Demand Using Node Embeddings)
 * 
 * Implements Requirement.md line 7: "edges transform locally in the same LLM transform globally"
 * MEMORY-EFFICIENT: Uses node embeddings as Q, K, V on-demand (no per-edge storage)
 * 
 * Key operations:
 * - Q * K / sqrt(d) attention score computation (Q=from_node, K=to_node embeddings)
 * - Single-head attention (simplified, no multi-head storage)
 * - Edge weight updates based on success (Hebbian learning)
 * 
 ...
 * MEMORY-EFFICIENT: Uses node embeddings on-demand (no EdgeTransformer storage)
 * LAZY INITIALIZATION: Creates embeddings if they don't exist
 */
static float edge_compute_attention_score(Edge *edge, SparseEmbedding *query_context) {
    if (!edge || !edge->from_node || !edge->to_node) return 1.0f;
    
    // Use node embeddings as Q and K (lazy initialization)
    // Query: use provided context, or from_node embedding (create if needed)
    SparseEmbedding *query = query_context ? query_context : node_ensure_sparse_embedding(edge->from_node);
    // Key: always use to_node embedding (create if needed)
    SparseEmbedding *key = node_ensure_sparse_embedding(edge->to_node);
    
    // If embeddings unavailable (allocation failed), return neutral score
    if (!query || !key || query->active_count == 0 || key->active_count == 0) {
        return 1.0f;
    }
    
    // Compute Q * K similarity (O(k) sparse dot product)
    float dot_product = sparse_embedding_similarity(query, key);
    
    // Scale by sqrt(d) where d = active dimensions
    size_t d = (query->active_count > key->active_count) ? query->active_count : key->active_count;
    float scale = 1.0f / sqrtf((float)d + 1.0f);
...
/* Multi-head attention: simplified to single-head using node embeddings - O(k)
 * MEMORY-EFFICIENT: No multi-head storage, uses node embeddings on-demand
 */
static float edge_multi_head_attention(Edge *edge, SparseEmbedding *query_context) {
    if (!edge) return 1.0f;
    
    // Compute attention score using node embeddings (on-demand, no storage)
    float attention_score = edge_compute_attention_score(edge, query_context);
    
    // Boost attention for edges with context tags (learned context matching)
    if (edge->tag_count > 0) {
        // Context tags indicate this edge has learned context-specific patterns
        float context_boost = 1.0f + (0.5f / (1.0f + (float)edge->tag_count));
...
/* Update node embeddings via Hebbian learning - O(k)
 * MEMORY-EFFICIENT: Updates node embeddings directly (no EdgeTransformer storage)
 * Learning happens at the node level, edges just strengthen based on success
 */
static void edge_update_projections(Edge *edge, SparseEmbedding *from_emb, SparseEmbedding *to_emb, float success) {
    if (!edge || !from_emb || !to_emb) return;
    
    // Compute adaptive learning rate based on success
    // Successful predictions strengthen more
    float learning_rate = 0.05f * (0.5f + success);
    
    // Update edge weight based on success (Hebbian: fire together, wire together)
    // Scale by 255 since edge weight is uint8_t
    float weight_delta = learning_rate * success * 255.0f;
    int new_weight = (int)edge->weight + (int)weight_delta;
    ...
    // Note: Node embeddings are updated elsewhere via sparse_embedding_update_hebbian()
    // during normal activation and learning. This function focuses on edge weight updates.
}
 ...
 * ============================================================================
 * 
 * This uses SparseContext and ContextTag for context-aware edge weighting.
 * See SparseContext, ContextTag, and related functions defined earlier.
 * 
 * System:
...
static Edge* node_predict_next_edge_sparse(Node *node, SparseContext *current_context) {
    ...
    // Requirement.md line 8: "nodes make mini prediction"
    // Use node's mini-net to predict which edge to take
    // Mini-net learns how to combine factors from data (not hardcoded)
    
    if (!node->net) {
        // Fallback: if no mini-net, use strongest edge
        Edge *best_edge = NULL;
        uint8_t max_weight = 0;
        for (size_t i = 0; i < node->outgoing_count; i++) {
            Edge *edge = node->outgoing_edges[i];
            if (!edge || !edge->to_node || edge->to_node->payload_size == 0) continue;
            // Include all edge types in fallback (similarity, context, homeostatic)
            // Previously skipped similarity edges, but now they're part of decision-making
    ...
    // === ORGANIC APPROACH: No fallbacks, use edge weights directly ===
    // Edge weights already learned from Hebbian learning during training
    // Mini-net refines the score additively, not replaces it
    // Requirement.md: "No Fallbacks" - everything relative and data-driven
    
    Edge *best_edge = NULL;
    float best_score = -INFINITY;
    
    // Compute local average for normalization (relative to local context)
    ...
    for (size_t i = 0; i < node->outgoing_count; i++) {
        ...
        // === PASSIVE CONTEXT: Compute from existing data (no storage) ===
        // Signal 1: Target activation (wave propagation context)
        float target_activation = edge->to_node->activation_strength;
        
        // Signal 2: Embedding similarity (semantic context)
        float embedding_sim = 0.0f;
        if (node && edge->to_node) {
            SparseEmbedding *from_emb = node_ensure_sparse_embedding(node);
            SparseEmbedding *to_emb = node_ensure_sparse_embedding(edge->to_node);
            if (from_emb && to_emb && from_emb->active_count > 0 && to_emb->active_count > 0) {
                embedding_sim = sparse_embedding_similarity(from_emb, to_emb);
            }
        }
        
        // Start with learned edge weight (base signal)
        float score = base_relative;
        
        // Add passive context signals (all additive, no hardcoded weights)
        ...
        if (node->net) {
            float inputs[] = {
                target_activation,  // Raw: target node activation from wave
                base_relative,      // Raw: edge weight relative to local avg
                embedding_sim,      // Raw: semantic similarity
                local_avg,          // Context: local average for normalization
                0.0f,               // Reserved for future signals
                0.0f                // Reserved for future signals
            };
            
            // Mini-net learns to refine the base score (additive refinement)
            // No hardcoded formulas - relationship emerges from experience
...
static Node* hierarchy_predict_continuation(Node *hierarchy, uint8_t *input_bytes, size_t input_len) {
    ...
    if (hierarchy->outgoing_count > 0) {
        ...
        for (size_t i = 0; i < hierarchy->outgoing_count; i++) {
            ...
            // REQUIREMENT COMPLIANCE: Use relative values
            float context_boost = edge_compute_context_weight(edge, NULL);  // Relative (no context = 1.0)
            float base_weight = (float)edge->weight / 255.0f;
            ...
            float base_relative = (hier_local_avg > hier_epsilon) ? 
                (base_weight / hier_local_avg) : base_weight;
            float context_relative = base_relative * context_boost;
            float score = context_relative;
...
static Node* blank_predict_for_category(Node *blank, Node *current_node, 
    ...
    for (size_t i = 0; i < blank->outgoing_count; i++) {
        ...
        // === BRAIN-INSPIRED CONTEXT SCORING ===
        // REQUIREMENT COMPLIANCE: Use relative values
        float context_boost = edge_compute_context_weight(edge, NULL);  // Relative
        ...
        float base_weight = (float)edge->weight / 255.0f;
        float base_relative = (blank_local_avg > blank_epsilon) ? 
            (base_weight / blank_local_avg) : base_weight;
        float context_relative = base_relative * context_boost;
        
        // Activation contributes proportionally
...
static Node* node_create(const uint8_t *payload, size_t payload_size, uint32_t abstraction_level) {
    ...
    // Organic format: 6 inputs (context_match_quality, base_relative, activation, local_avg, context_count, context_level)
    node->net = mini_net_create(6, 8);  // 6 inputs: context_match, base_relative, activation, local_avg, context_count, context_level
    if (!node->net) {
        free(node->payload);
        free(node);
        return NULL;
    ...
    // CONTEXT: No per-node storage - context is graph-wide SparseContext
    // Context is passed as ActivationPattern/SparseContext to functions
    // This enables infinite context that scales to billions of nodes
...
static void graph_create_similarity_edges_for_node(Graph *graph, Node *new_node, SparseContext *training_ctx);
static float compute_input_activation(Node *input_node);
static void compute_similarity_edge_range(Node *node, float *min_relative, float *max_relative);
...
static void node_self_destruct(Node *node) {
    ...
    // Context is graph-wide SparseContext - no per-node storage to free
...
static float compute_node_bloat_score(Node *node) {
    ...
    // Factor 4: Low activation history (never used in meaningful sequences)
    // Context is now graph-wide (SparseContext), not per-node
    // Use activation count as proxy for context participation
    float context_factor = (float)node->total_activations / ((float)node->total_activations + 10.0f);
    float contextless_factor = 1.0f - context_factor;  // High when rarely activated
 ...
 */
/* REMOVED: edge_add_context_tag() - no longer needed
 * Context is now passive (computed from activation, not stored)
 */

/* PASSIVE CONTEXT: Compute context signal from edge and activation pattern
 * Brain: Context emerges from which neurons are active (no storage)
 * 
 * Requirement.md line 11: "context changes the edge weights of the current node"
 * - Context = current activation pattern (which nodes are active)
 * - Edge weight modulated by target activation (passive, no storage)
 * 
 * Returns: Context signal (additive contribution to edge score)
 */
static float edge_compute_passive_context_signal(Edge *edge, ActivationPattern *pattern) {
    if (!edge || !edge->to_node) return 0.0f;
    if (!pattern || pattern->count == 0) return 0.0f;
    
    // Signal 1: Target node activation (from wave propagation)
    // If target is already active, this edge is contextually relevant
    float target_activation = edge->to_node->activation_strength;
    
    // Signal 2: Embedding similarity (semantic context)
    // If target is semantically similar to source, this edge is relevant
    float embedding_signal = 0.0f;
    if (edge->from_node && edge->to_node) {
        SparseEmbedding *from_emb = node_ensure_sparse_embedding(edge->from_node);
        SparseEmbedding *to_emb = node_ensure_sparse_embedding(edge->to_node);
        if (from_emb && to_emb && from_emb->active_count > 0 && to_emb->active_count > 0) {
            embedding_signal = sparse_embedding_similarity(from_emb, to_emb);
        }
    }
    
    // Signal 3: Co-activation signal (how many active nodes connect to target?)
    // If many active nodes point to target, it's contextually supported
    float co_activation = 0.0f;
    size_t connections = 0;
    for (size_t i = 0; i < pattern->count && i < 20; i++) {  // Limit to 20 for efficiency
        Node *active = pattern->nodes[i];
        if (!active) continue;
        
        // Check if active node has edge to target
        for (size_t j = 0; j < active->outgoing_count; j++) {
            if (active->outgoing_edges[j] && 
                active->outgoing_edges[j]->to_node == edge->to_node) {
                co_activation += pattern->activations[i];
                connections++;
                break;
            }
        }
    ...
    // Normalize co-activation by number of connections
    if (connections > 0) {
        co_activation /= (float)connections;
    }
    
    // Combine signals (additive, all contribute equally)
    // Mini-net will learn which to prioritize
    float context_signal = target_activation + embedding_signal + co_activation;
    
    return context_signal;
}

/* PASSIVE CONTEXT WRAPPER: Replaces old edge_compute_context_weight()
 * Now uses passive computation instead of stored context tags
 * Kept for backward compatibility with existing code
 */
static float edge_compute_context_weight(Edge *edge, SparseContext *current_context) {
    if (!edge) return 0.0f;
    
    // PASSIVE CONTEXT: Ignore current_context parameter (deprecated)
    // Instead, use target node's current activation (passive, no storage)
    (void)current_context;  // Unused - context is now passive
    
    // Signal 1: Target activation (wave propagation context)
    float target_activation = edge->to_node ? edge->to_node->activation_strength : 0.0f;
    ...
    // Return combined passive context signal
    // No storage, all derived from current state
    return target_activation + embedding_sim;
}

/* REMOVED: edge_prune_context_tags() - no longer needed
 * Context is now passive (computed from activation, not stored)
 */
static void edge_prune_context_tags_removed(Edge *edge) {
    if (!edge) return;
    ...
    // ADAPTIVE PRUNING: No hardcoded limits (Requirement.md compliance)
    // Compute data-driven threshold from tag distribution
    // Keep tags that are above median weight contribution (adaptive threshold)
    float *weights = malloc(edge->tag_count * sizeof(float));
    if (!weights) return;  // Can't prune if no memory
    
    for (size_t i = 0; i < edge->tag_count; i++) {
        weights[i] = edge->context_tags[i].weight_contribution;
    }
    
    // Sort to find median (simple bubble sort for small arrays, O(k^2) where k is small)
    for (size_t i = 0; i < edge->tag_count - 1; i++) {
        for (size_t j = 0; j < edge->tag_count - i - 1; j++) {
    ...
    // Median threshold (data-driven, no hardcoded value)
    float median_weight = (edge->tag_count > 0) ? 
        weights[edge->tag_count / 2] : avg_weight;
    free(weights);
    
    // Keep tags above median (adaptive threshold based on actual data)
    size_t write_idx = 0;
    for (size_t i = 0; i < edge->tag_count; i++) {
        float relative_weight = edge->context_tags[i].weight_contribution / (median_weight + 1e-6f);
        // Adaptive threshold: keep tags above median (data-driven, not hardcoded)
        if (relative_weight >= 1.0f) {
            // Keep this tag (above median)
            if (write_idx != i) {
                edge->context_tags[write_idx] = edge->context_tags[i];
            }
            write_idx++;
        } else {
            // Prune this tag (below median)
            sparse_context_free(edge->context_tags[i].context);
        }
    }
...
static float edge_transform_activation(Edge *edge, float input_activation, Graph *graph) {
    ...
    // TRANSFORMER ATTENTION (on-demand, no storage)
    // Implements Requirement.md line 7: "edges transform locally in the same LLM transform globally"
    // Uses node embeddings as Q, K, V (computed on-demand, no EdgeTransformer storage)
    // - Query (Q) = from_node->sparse_embedding (what source is asking for)
    // - Key (K) = to_node->sparse_embedding (what target matches against)
    // - Attention = Q·K similarity (sparse dot product, O(k))
    // - Output = weight × attention × activation
    
    // Base transformation: normalize uint8_t weight to 0.0-1.0 range
    float weight_norm = (float)edge->weight / 255.0f;
    
    // Compute transformer attention on-demand using node embeddings
    float attention_score = 1.0f;  // Default: no attention boost
    
    // Use node embeddings as Q, K (computed on-demand, no storage needed)
    SparseEmbedding *query = edge->from_node->sparse_embedding;
    SparseEmbedding *key = edge->to_node->sparse_embedding;
    
    // If embeddings exist, compute transformer attention (Q·K / sqrt(d))
    if (query && key && query->active_count > 0 && key->active_count > 0) {
        // Compute Q·K similarity (sparse dot product, O(k))
        float dot_product = sparse_embedding_similarity(query, key);
        
        // Scale by sqrt(d) where d = active dimensions (standard transformer scaling)
        size_t d = (query->active_count > key->active_count) ? query->active_count : key->active_count;
        float scale = 1.0f / sqrtf((float)d + 1.0f);
        
        // Attention score: Q·K / sqrt(d) with sigmoid normalization
        float raw_attention = dot_product * scale;
        attention_score = 1.0f / (1.0f + expf(-raw_attention));  // Sigmoid: range [0, 1]
        
        // Boost attention for edges with context tags (learned context matching)
        if (edge->tag_count > 0) {
            // Context tags indicate this edge has learned context-specific patterns
            // Boost attention when context matches
            float context_boost = 1.0f + (1.0f / (1.0f + (float)edge->tag_count));
            attention_score *= context_boost;
        }
    } else {
        // No embeddings yet: use simple gate based on context tags
        float gate_norm = (edge->tag_count > 0) ? 1.0f : 0.5f;
        attention_score = gate_norm;
    }
    
    // Transformer output: weight × attention × activation
    // This implements "transform locally" - each edge transforms based on semantic similarity
    return weight_norm * attention_score * input_activation;
}
...
static float edge_score_for_routing(Edge *edge, float source_activation, float local_avg_weight) {
    ...
    // All edges are unified - weight and context encode their purpose
    float similarity_priority = 1.0f;
    {
        // Priority computed from edge weight relative to local average
...
static void edge_update_weight(Edge *edge, float activation) {
    ...
    // PASSIVE CONTEXT: No pruning needed (no context storage)
...
static void node_update_context_values(Node *node, uint32_t current_generation) {
    ...
    // CONTEXT: Now uses graph-wide SparseContext (infinite context, no per-node storage)
    // Context is passed as ActivationPattern/SparseContext to functions
    // No size adaptation needed - context grows with graph
...
static Edge* node_compute_winning_edge_with_context(Node *node, Graph *graph,
    ...
    // CONTEXT: Create SparseContext from input + output nodes + wave_state (infinite context)
    // Context = all processed nodes so far (matches training context tags)
    // CRITICAL FIX: Include wave_state activated nodes for better context matching
    // This enables accurate disambiguation by using the full activation pattern
    SparseContext *current_ctx = NULL;
    
    // Build combined context from input + output nodes + wave_state activated nodes
    size_t wave_activated_count = (wave_state && wave_state->all_activated_nodes) ? 
                                   wave_state->all_activated_count : 0;
    size_t total_ctx_count = input_count + output_nodes_len + wave_activated_count;
    if (total_ctx_count > 0) {
        ...
        if (ctx_nodes && ctx_activations) {
            size_t idx = 0;
            
            // Add input nodes with decreasing activation (older = less activation)
            for (size_t i = 0; i < input_count && input_nodes; i++) {
                ctx_nodes[idx] = input_nodes[i];
                ctx_activations[idx] = 1.0f - (float)i / (float)total_ctx_count * 0.5f;
                idx++;
            }
            
            // Add output nodes (more recent = stronger activation)
            for (size_t i = 0; i < output_nodes_len && output_nodes; i++) {
                ctx_nodes[idx] = output_nodes[i];
                ctx_activations[idx] = 1.0f - (float)(input_count + i) / (float)total_ctx_count * 0.5f;
                idx++;
            }
            
            // CRITICAL: Add wave_state activated nodes (full activation pattern)
            // This includes all nodes activated during wave propagation, providing rich context
            if (wave_state && wave_state->all_activated_nodes && 
                wave_state->all_activation_strengths) {
                for (size_t i = 0; i < wave_activated_count && idx < total_ctx_count; i++) {
                    // Only add if not already in context (avoid duplicates)
                    int already_added = 0;
                    for (size_t j = 0; j < idx; j++) {
                        if (ctx_nodes[j] == wave_state->all_activated_nodes[i]) {
                            already_added = 1;
                            break;
                        }
                    }
                    if (!already_added) {
                        ctx_nodes[idx] = wave_state->all_activated_nodes[i];
                        ctx_activations[idx] = wave_state->all_activation_strengths[i];
                        idx++;
                    }
                }
            }
    ...
    // Note: node_update_context_values() will use SparseContext via edge_compute_context_weight()
    node_update_context_values(node, graph ? graph->wave_generation : 0);
    
    // Use context-gated edge values with SparseContext matching
    float best_value = -1.0f;
    Edge *best = NULL;
    ...
    for (size_t i = 0; i < node->outgoing_count; i++) {
        ...
        // === ORGANIC APPROACH: Raw signals, no hardcoded formulas ===
        // Get context match quality (raw signal)
        float context_match_quality = 1.0f;  // Default: no context
        if (current_ctx) {
            context_match_quality = edge_compute_context_weight(edge, current_ctx);
        }
        
        // Get edge weight relative to local avg (raw signal)
        float base_weight = (float)edge->weight / 255.0f;
        float local_avg = node_get_local_outgoing_weight_avg(node);
        float epsilon = compute_adaptive_epsilon(local_avg);
        float base_relative = (local_avg > epsilon) ? (base_weight / local_avg) : base_weight;
        
        // Combine raw signals (simple sum - no hardcoded formula)
        // Both signals contribute, competition happens naturally
        float context_value = context_match_quality + base_relative;
        
        // Apply habituation (penalize recent repetitions)
        // Check ALL output_nodes (no hardcoded window - organic)
        if (output_nodes && output_len > 0) {
            ...
            if (recent_count > 0) {
                // Habituation strength adapts to repetition count (data-driven)
    ...
    // ORGANIC: Stop competes with edges locally (like any other decision)
    // If node's learned stop_weight is stronger than best edge, stop wins
    if (node && node->stop_weight > 0.0f) {
        float stop_value = node->stop_weight;
        
        // Stop competes with best edge value
        if (stop_value > best_value) {
            // Stop wins - return NULL to signal natural stopping point
            if (current_ctx) sparse_context_free(current_ctx);
            return NULL;
        }
    }
    
    // If no edges found, natural stop (node has no continuation)
    if (!best && node && node->outgoing_count == 0) {
        if (current_ctx) sparse_context_free(current_ctx);
        return NULL;
    }
    
    // PASSIVE CONTEXT: No context to free
...
static Graph* graph_create(void) {
    ...
    // === CREATE STOP NODE (Like LLM EOS Token) ===
    // Every node can have an edge to STOP, trained like any other edge
    // During generation, STOP edge competes with regular edges
    // If STOP edge wins, generation stops (like EOS token in LLMs)
    // Use a special pattern node with a unique byte (0xFF) to represent STOP
    // This ensures it's distinct from regular data patterns
    uint8_t stop_pattern[] = {0xFF};  // Special byte for STOP node
    graph->stop_node = graph_find_or_create_pattern_node(graph, stop_pattern, 1);
    // If creation failed, we'll handle it gracefully during edge creation
...
static ActivationPattern* encode_input_spreading(Graph *graph, Node **input_nodes, size_t input_count) {
    ...
    for (size_t i = 0; i < input_count; i++) {
        ...
        // CONTEXT: Now uses graph-wide SparseContext (infinite context)
        // Context is the entire ActivationPattern - no per-node storage needed
        // Disambiguation happens via SparseContext matching in edge selection
    ...
    // When input matches hierarchy prefix, activate the hierarchy
    // This enables Phase 2 to form larger hierarchies and generation to use them
    if (graph && graph->hierarchy_by_level && graph->max_hierarchy_levels > 0) {
        // Build input byte sequence for matching
        size_t input_bytes_capacity = input_count * 2;  // Safety buffer
        uint8_t *input_bytes = malloc(input_bytes_capacity);
        size_t input_bytes_len = 0;
        
        if (input_bytes) {
            // Collect bytes from input nodes
            for (size_t i = 0; i < input_count && input_bytes_len < input_bytes_capacity; i++) {
                if (input_nodes[i] && input_nodes[i]->payload_size > 0) {
                    size_t payload_size;
                    uint8_t *payload = node_get_payload(input_nodes[i], &payload_size);
                    if (payload) {
                        size_t copy_size = (input_bytes_len + payload_size < input_bytes_capacity) 
                                         ? payload_size : (input_bytes_capacity - input_bytes_len);
                        memcpy(input_bytes + input_bytes_len, payload, copy_size);
                        input_bytes_len += copy_size;
                        
                        // Free if dynamically allocated (reference hierarchy)
                        if (input_nodes[i]->is_reference_hierarchy) {
                            free(payload);
                        }
                    }
                }
            ...
            // Search hierarchies from highest to lowest level (longer patterns first)
            for (uint32_t level = graph->max_abstraction_level; level > 0; level--) {
                if (level >= graph->max_hierarchy_levels) continue;
                if (!graph->hierarchy_by_level[level]) continue;
                ...
                for (size_t h = 0; h < count; h++) {
                    ...
                    // Get hierarchy payload
                    size_t hier_size;
                    uint8_t *hier_payload = node_get_payload(hierarchy, &hier_size);
                    if (!hier_payload || hier_size == 0) continue;
                    
                    // Check if input matches hierarchy prefix
                    size_t match_len = (input_bytes_len < hier_size) ? input_bytes_len : hier_size;
                    int matches = 1;
                    for (size_t j = 0; j < match_len; j++) {
                        if (input_bytes[j] != hier_payload[j]) {
                            matches = 0;
                            break;
                        }
                    }
                    
                    // Free hierarchy payload if dynamically allocated
                    if (hierarchy->is_reference_hierarchy) {
                        free(hier_payload);
                    }
                    
                    // If input matches hierarchy prefix, activate the hierarchy
                    if (matches && match_len > 0) {
                        // Activation strength based on match length and hierarchy level
                        float match_ratio = (float)match_len / (float)hier_size;
                        float level_boost = 1.0f + (float)hierarchy->abstraction_level * 0.1f;
                        float hierarchy_activation = match_ratio * level_boost;
                        
                        // Add hierarchy to activation pattern
...
static float score_candidate_with_all_mechanisms(
    ...
    // Context handled by node_update_context_values()
...
static void generate_from_pattern(MFile *mfile, ActivationPattern *pattern, 
    ...
    // FIX 2: Include ALL bytes from input nodes (not just first byte)
    // This enables proper hierarchy matching for multi-byte inputs
    for (size_t i = 0; i < input_count && context_len < full_context_capacity; i++) {
        if (input_nodes[i]) {
            size_t payload_size;
            uint8_t *payload = node_get_payload(input_nodes[i], &payload_size);
            if (payload && payload_size > 0) {
                size_t copy_size = (context_len + payload_size < full_context_capacity) 
                                 ? payload_size : (full_context_capacity - context_len);
    ...
    */
    ...
    if (!current_node && input_nodes && input_count > 0) {
        ...
        if (last_input && last_input->outgoing_edges && last_input->outgoing_count > 0) {
            ...
            for (size_t i = 0; i < last_input->outgoing_count; i++) {
                ...
                // PRINCIPLE 4: Hierarchies enable compounding learning - let them compete
                // Hierarchies represent learned patterns and should be used for generation
                // They're already handled at Priority 1 (hierarchy guidance), but can also compete here
                
                // REQUIREMENT COMPLIANCE: Use relative values
                float context_boost = edge_compute_context_weight(edge, current_ctx);  // Relative
                float base_weight = (float)edge->weight / 255.0f;
                ...
                float score;
                // === ORGANIC APPROACH: No fallbacks, use edge weights directly ===
                // Edge weights already learned from Hebbian learning
                // Mini-net refines additively, not replaces
                
                // Base score: Edge weight relative to local average (already learned)
                score = base_relative;
                
                // Mini-net refines the score additively (learns to adjust, not replace)
                if (last_input->net) {
                    float inputs[] = {
                        context_relative,   // ✅ RELATIVE (context_match_quality equivalent)
                        base_relative,      // ✅ RELATIVE
                        activation,         // ✅ RELATIVE
                        context_boost,      // ✅ RELATIVE (additional context signal)
                        local_avg,          // ✅ CONTEXT
                        (float)input_count / 100.0f  // Context count
                    };
                    float refinement = mini_net_forward(last_input->net, inputs, 6, NULL, 0);
                    score += refinement;  // Additive refinement
                }
    ...
    while (current_node && current_node->payload_size > 0 && output_len < max_output_len) {
        ...
        // PRINCIPLE 4: Hierarchies enable compounding learning - they should compete like any node
        // PRINCIPLE 8: Hierarchies are explicit nodes - they should be used for generation
        if (!next_node && current_node->net) {
            Edge *predicted_edge = node_predict_next_edge(current_node, full_context, context_len);
            // Let hierarchies compete - they represent learned patterns and should guide generation
        ...
        // === PRIORITY 4: PROBABILITY-BASED EDGE SELECTION ===
        // SOFTMAX over all edges (like LLM next-token prediction)
        // Each edge gets a probability based on its score
        // Sample from distribution (probabilistic, not deterministic)
        if (!next_node) {
            ...
            // Collect all candidates and scores (for softmax)
            typedef struct {
                Edge *edge;
                Node *target;
                float score;
            ...
            EdgeCandidate *candidates = malloc(current_node->outgoing_count * sizeof(EdgeCandidate));
            if (!candidates) {
                // PASSIVE CONTEXT: No context to free
            ...
            for (size_t i = 0; i < current_node->outgoing_count; i++) {
                ...
                // === ORGANIC APPROACH: Raw signals, no hardcoded formulas ===
                // Get context match quality (raw signal)
                float context_match_quality = edge_compute_context_weight(edge, current_ctx);
                ...
                float base_relative = (local_avg_gen > epsilon_gen) ? 
                    (base_weight / local_avg_gen) : base_weight;
                
                // Get target activation (raw signal)
                ...
                // Mini-net refines the score additively (learns to adjust, not replace)
                if (current_node->net) {
                    float context_count = (current_ctx && current_ctx->count > 0) ? 
                                          (float)current_ctx->count / 100.0f : 0.0f;
                    float context_level = (current_ctx && current_ctx->count > 0) ? 
                                          (float)current_ctx->max_abstraction_level / 10.0f : 0.0f;
                    
                    float inputs[] = {
                        context_match_quality,  // Raw: how well edge matches context
                        base_relative,          // Raw: edge weight relative to local avg
                        activation,             // Raw: target node activation
                        local_avg_gen,          // Context: local average for normalization
                        context_count,          // Context: size of current context
                        context_level           // Context: abstraction level
                    };
                    // Mini-net learns from experience which signals to prioritize
                    float refinement = mini_net_forward(current_node->net, inputs, 6, NULL, 0);
                    score += refinement;  // Additive refinement
                }
                
                // Store candidate (don't select yet - will use softmax)
                candidates[candidate_count].edge = edge;
                candidates[candidate_count].target = target;
                candidates[candidate_count].score = score;
                candidate_count++;
            }
            
            // SOFTMAX + SAMPLING (probabilistic selection)
            if (candidate_count > 0) {
                ...
                float temperature = 1.0f;  // Can be adaptive later
                float sum_exp = 0.0f;
                for (size_t i = 0; i < candidate_count; i++) {
                    candidates[i].score = expf((candidates[i].score - max_score) / temperature);
                    sum_exp += candidates[i].score;
                }
                
                // Normalize to probabilities
                for (size_t i = 0; i < candidate_count; i++) {
                    candidates[i].score /= sum_exp;
                }
                ...
                float cumsum = 0.0f;
                for (size_t i = 0; i < candidate_count; i++) {
                    cumsum += candidates[i].score;
                    if (rand_val <= cumsum) {
                        next_node = candidates[i].target;
                        used_edge = candidates[i].edge;
                        break;
                    }
                }
            }
        ...
        // Note: STOP edge competition is now integrated into softmax above
        // If no next_node selected, generation will naturally stop
        ...
        // CONTEXT: Now uses graph-wide SparseContext (infinite context)
        // No per-node context_trace updates needed - context is the ActivationPattern
        ...
        // CONTEXT: Now uses graph-wide SparseContext (infinite context)
        // Context is the entire ActivationPattern - no per-node updates needed
        // Different contexts are distinguished via SparseContext matching in edge selection
 ...
 */
static void graph_create_similarity_edges_for_node(Graph *graph, Node *new_node, SparseContext *training_ctx) {
    if (!graph || !new_node || !new_node->payload || new_node->payload_size == 0) return;
    
    // Don't create similarity edges for blank nodes (they have no payload)
    if (new_node->payload_size == 0) return;
    
    // REQUIREMENT COMPLIANCE: NO O(n) searches - only follow edges
    // Nodes can only see nodes reachable through edges (Requirement.md line 2, line 7)
    // Check nodes that are 1-2 hops away through existing edges
    
    size_t edges_created = 0;
    size_t max_similarity_edges = 10;
    
    // HOP 1: Check nodes directly connected to new_node (outgoing edges)
    for (size_t i = 0; i < new_node->outgoing_count && edges_created < max_similarity_edges; i++) {
        Edge *edge = new_node->outgoing_edges[i];
        if (!edge || !edge->to_node) continue;
        
        Node *connected = edge->to_node;
        if (!connected->payload || connected->payload_size == 0) continue;
        
        // Check if similarity edge already exists
        if (graph_find_edge_between(new_node, connected)) continue;
        
        // Compute payload similarity
        size_t min_size = (new_node->payload_size < connected->payload_size) ? 
                          new_node->payload_size : connected->payload_size;
        size_t max_size = (new_node->payload_size > connected->payload_size) ? 
                          new_node->payload_size : connected->payload_size;
        
        if (min_size == 0 || max_size == 0) continue;
        
        size_t matching_bytes = 0;
        for (size_t j = 0; j < min_size; j++) {
            if (new_node->payload[j] == connected->payload[j]) {
                matching_bytes++;
            }
        }
        
        float similarity = (float)matching_bytes / (float)max_size;
        float threshold = 0.3f;
        
        if (similarity > threshold) {
            // Create bidirectional similarity edges
            Edge *edge1 = edge_create(new_node, connected);
            Edge *edge2 = edge_create(connected, new_node);
            
            if (edge1) {
                edge1->weight = (uint8_t)(similarity * 128.0f);
                graph_add_edge(graph, edge1);
                edges_created++;
            }
            
            if (edge2) {
                edge2->weight = (uint8_t)(similarity * 128.0f);
    ...
    for (size_t i = 0; i < new_node->outgoing_count && edges_created < max_similarity_edges; i++) {
        Edge *edge1 = new_node->outgoing_edges[i];
        if (!edge1 || !edge1->to_node) continue;
        
        Node *neighbor = edge1->to_node;
        
        // Check neighbors of this neighbor
        for (size_t j = 0; j < neighbor->outgoing_count && edges_created < max_similarity_edges; j++) {
            Edge *edge2 = neighbor->outgoing_edges[j];
            if (!edge2 || !edge2->to_node) continue;
            
            Node *other = edge2->to_node;
            if (other == new_node || !other->payload || other->payload_size == 0) continue;
            
            // Skip if edge already exists
            if (graph_find_edge_between(new_node, other)) continue;
            
            // Compute payload similarity
            size_t min_size = (new_node->payload_size < other->payload_size) ? 
                              new_node->payload_size : other->payload_size;
            size_t max_size = (new_node->payload_size > other->payload_size) ? 
                              new_node->payload_size : other->payload_size;
            ...
            for (size_t k = 0; k < min_size; k++) {
                if (new_node->payload[k] == other->payload[k]) {
                    matching_bytes++;
                }
            }
            ...
            if (similarity > threshold) {
                Edge *edge1_new = edge_create(new_node, other);
                Edge *edge2_new = edge_create(other, new_node);
                
                if (edge1_new) {
                    edge1_new->weight = (uint8_t)(similarity * 128.0f);
                    graph_add_edge(graph, edge1_new);
                    edges_created++;
                }
                
                if (edge2_new) {
                    edge2_new->weight = (uint8_t)(similarity * 128.0f);
                    graph_add_edge(graph, edge2_new);
                }
            }
        }
    }
...
/* Create context edges from wave propagation (connects co-activating nodes)
 * - Brain: Neurons that activate together form connections
 * - Creates edges between nodes in same activation pattern
 * - Enables contextual associations
 * - O(k^2) where k = activated nodes (typically 10-50)
 */
static void graph_create_context_edges_from_pattern(Graph *graph, ActivationPattern *pattern, SparseContext *training_ctx) {
    if (!graph || !pattern || pattern->count < 2) return;
    
    // Adaptive limit: connect each node to a few others in pattern
    size_t max_connections_per_node = 5;  // Each node connects to ~5 others in context
    
    for (size_t i = 0; i < pattern->count; i++) {
        Node *from = pattern->nodes[i];
        if (!from || !from->payload || from->payload_size == 0) continue;
        
        float from_activation = pattern->activations[i];
        if (from_activation < 0.1f) continue;  // Only connect strongly activated nodes
        
        size_t connections_made = 0;
        
        // Connect to other nodes in pattern
        for (size_t j = 0; j < pattern->count && connections_made < max_connections_per_node; j++) {
            if (i == j) continue;  // Skip self
            
            Node *to = pattern->nodes[j];
            if (!to || !to->payload || to->payload_size == 0) continue;
            
            float to_activation = pattern->activations[j];
            if (to_activation < 0.1f) continue;
            
            // Skip if edge already exists
            if (graph_find_edge_between(from, to)) continue;
            
            // Create context edge
            Edge *edge = edge_create(from, to);
            if (edge) {
                // Weight based on co-activation strength
                float co_activation = (from_activation + to_activation) / 2.0f;
                edge->weight = (uint8_t)(co_activation * 64.0f);  // Weak-medium weight
                // PASSIVE CONTEXT: No context tags needed (computed from activation)
...
/* Create homeostatic edges to prevent node isolation
 * - Brain: Homeostatic plasticity maintains minimum connectivity
 * - Ensures every node has at least minimum connections
 * - Connects to nearby nodes in graph
 * - O(k) where k = nodes to check
 */
static void graph_create_homeostatic_edges_for_node(Graph *graph, Node *node) {
    if (!graph || !node) return;
    
    // Adaptive minimum: nodes should have at least 3-5 outgoing edges
    size_t min_outgoing = 3;
    
    if (node->outgoing_count >= min_outgoing) return;  // Already connected
    
    // REQUIREMENT COMPLIANCE: NO O(n) searches - only follow edges
    // Nodes can only see nodes reachable through edges (Requirement.md line 2, line 7)
    // Connect to neighbors of neighbors (2 hops away) to maintain minimum connectivity
    
    size_t needed = min_outgoing - node->outgoing_count;
    size_t connections_made = 0;
    
    // HOP 1: If node has any outgoing edges, connect to neighbors of those neighbors
    if (node->outgoing_count > 0) {
        for (size_t i = 0; i < node->outgoing_count && connections_made < needed; i++) {
            Edge *edge1 = node->outgoing_edges[i];
            if (!edge1 || !edge1->to_node) continue;
            
            Node *neighbor = edge1->to_node;
            
            // Check neighbors of this neighbor (2 hops away)
            for (size_t j = 0; j < neighbor->outgoing_count && connections_made < needed; j++) {
                Edge *edge2 = neighbor->outgoing_edges[j];
                if (!edge2 || !edge2->to_node) continue;
                ...
                // Skip if edge already exists
                if (graph_find_edge_between(node, other)) continue;
                
                // Create homeostatic edge
                Edge *edge = edge_create(node, other);
                if (edge) {
                    edge->weight = 32;  // Weak weight (homeostatic, not learned)
                    graph_add_edge(graph, edge);
                    connections_made++;
                }
            }
    ...
    if (node->incoming_count > 0 && connections_made < needed) {
        for (size_t i = 0; i < node->incoming_count && connections_made < needed; i++) {
            Edge *edge1 = node->incoming_edges[i];
            if (!edge1 || !edge1->from_node) continue;
            
            Node *neighbor = edge1->from_node;
            
            // Check outgoing edges from this neighbor
            for (size_t j = 0; j < neighbor->outgoing_count && connections_made < needed; j++) {
                Edge *edge2 = neighbor->outgoing_edges[j];
                if (!edge2 || !edge2->to_node) continue;
    ...
    while (i < input_size) {
        ...
        if (prev_node) {
            ...
            if (processed_count > 0) {
                ...
                // PASSIVE CONTEXT: No longer creating training_ctx (not needed)
                training_ctx = NULL;
            }
            ...
            if (existing_edge) {
                ...
                // MINI-NET LEARNING: Train the node's mini-net on this successful edge selection
                // === ORGANIC APPROACH: Use same raw signals as prediction ===
                if (prev_node->net && training_ctx) {
                    // Use same organic format as prediction (6 inputs, raw signals)
                    float context_match_quality = edge_compute_context_weight(existing_edge, training_ctx);
                    float base_weight = (float)existing_edge->weight / 255.0f;
                    ...
                    // Positive outcome: this edge was correct (it appeared in training data)
                    mini_net_update(prev_node->net, inputs, 6, 1.0f, 1.0f);
                }
            } else {
                // Create new edge
                ...
                if (edge) {
                    ...
                    if (prev_node->net && training_ctx) {
                        // Use same organic format as prediction (6 inputs, raw signals)
                        float context_match_quality = edge_compute_context_weight(edge, training_ctx);
                        float base_weight = (float)edge->weight / 255.0f;
                        float local_avg = node_get_local_outgoing_weight_avg(prev_node);
                        float epsilon = (local_avg > 0.0f) ? (local_avg * 0.001f) : 0.001f;
                        float base_relative = (local_avg > epsilon) ? (base_weight / local_avg) : base_weight;
                        ...
                        // Positive outcome: new edge is correct
                        mini_net_update(prev_node->net, inputs, 6, 1.0f, 1.0f);
                    }
                }
            }
            
            // === STOP EDGE TRAINING (Like LLM EOS Token) ===
            // At the END of a training pattern, stopping was correct
            // Create/strengthen edge to STOP node (like training EOS token)
            // This teaches the system: "In this context, stopping is good"
            if (i == input_size - 1 && node && graph->stop_node) {
                // Find or create edge to STOP
                Edge *stop_edge = graph_find_edge_between(node, graph->stop_node);
                
                if (stop_edge) {
                    // Strengthen existing stop edge (like training EOS token)
                    // Use same strengthening logic as regular edges
                    float strengthening_rate = compute_adaptive_strengthening_rate(node);
                    float increment = (float)stop_edge->weight * (strengthening_rate - 1.0f);
                    // Update weight (bounded to 0-255)
                    float new_weight_float = (float)stop_edge->weight + increment;
                    if (new_weight_float > 255.0f) new_weight_float = 255.0f;
                    if (new_weight_float < 0.0f) new_weight_float = 0.0f;
                    stop_edge->weight = (uint8_t)new_weight_float;
                    
                    // PASSIVE CONTEXT: No context tags needed (computed from activation)
                } else {
                    // Create new stop edge
                    stop_edge = edge_create(node, graph->stop_node);
                    if (stop_edge) {
                        // PASSIVE CONTEXT: No context tags needed
                ...
                // === ORGANIC APPROACH: Use same raw signals as prediction ===
                if (node->net && training_ctx && stop_edge) {
                    // Use same organic format as prediction (6 inputs, raw signals)
                    float context_match_quality = edge_compute_context_weight(stop_edge, training_ctx);
                    float base_weight = (float)stop_edge->weight / 255.0f;
                    float local_avg = node_get_local_outgoing_weight_avg(node);
                    float epsilon = (local_avg > 0.0f) ? (local_avg * 0.001f) : 0.001f;
                    float base_relative = (local_avg > epsilon) ? (base_weight / local_avg) : base_weight;
                    float activation = 0.0f;  // STOP has no activation
                    float context_count = (float)training_ctx->count / 100.0f;
                    float context_level = (float)training_ctx->max_abstraction_level / 10.0f;
                    ...
                    float stop_inputs[] = {
                        context_match_quality,  // Raw: how well edge matches context
                        base_relative,          // Raw: edge weight relative to local avg
                        activation,             // Raw: target activation (0 for STOP)
                        local_avg,              // Context: local average for normalization
                        context_count,          // Context: size of current context
                        context_level           // Context: abstraction level
                    };
                    
                    // Positive outcome: stopping was correct (end of pattern)
                    mini_net_update(node->net, stop_inputs, 6, 1.0f, 1.0f);
                }
            }
            
            // PASSIVE CONTEXT: No training context to free (training_ctx = NULL)
...
/* Compute hierarchy formation probability (for learning, not blocking)
 * BRAIN-LIKE: Hierarchies form from pure competition (relative_strength > 1.0f)
 * Mini nets learn from outcomes but don't block formation
 * This function is used for learning/feedback, not decision-making
 */
static float compute_hierarchy_formation_probability_for_learning(Node *node1, Node *node2, Edge *connecting_edge, Graph *graph) {
    if (!node1 || !node2 || !graph) return 0.0f;
    
    // Compute formation probability using mini nets (for learning purposes)
    float formation_prob = compute_hierarchy_formation_probability(node1, node2, connecting_edge);
    
    // Graph's mini net can influence learning rate, not formation decision
    if (graph->hierarchy_net) {
        ...
        // Use mini net to influence learning, not block formation
        float learning_factor = mini_net_forward(graph->hierarchy_net, graph_context, 5, NULL, 0);
        formation_prob *= (0.5f + learning_factor);  // Scale by learning factor
    }
 ...
 * BRAIN-LIKE: Mini nets learn from outcomes but don't block formation
 * Formation is pure competition (relative_strength > 1.0f)
 * Mini nets learn: "Was this hierarchy useful?" (not "Should we form it?")
 */
static void learn_hierarchy_formation(Node *node1, Node *node2, Edge *connecting_edge, 
                                     float outcome, Graph *graph) {
    if (!node1 || !node2) return;
    
    // Build context vector (what conditions led to this hierarchy)
    float context[10];
    context[0] = node1->activation_strength;
    context[1] = node2->activation_strength;
    context[2] = connecting_edge ? (float)connecting_edge->weight : 0.0f;
    context[3] = (float)node1->total_activations;  // Usage frequency
    context[4] = (float)node2->total_activations;
    context[5] = (float)node1->abstraction_level;
    context[6] = (float)node2->abstraction_level;
    ...
    // Both nodes learn from the outcome (reinforcement learning)
    // Outcome = 1.0f if hierarchy was useful, 0.0f if not
    if (node1->net) {
    ...
    // Graph's mini net learns: "What graph states lead to useful hierarchies?"
    // This influences future learning rates, not formation decisions
...
static Node* create_hierarchy_node(Graph *graph, Node *node1, Node *node2) {
    ...
    // BRAIN-LIKE LEARNING: Mini nets learn from successful hierarchy formation
    // Positive outcome: hierarchy was formed (reinforcement learning)
...
/* Check if two nodes should form a hierarchy and create it if needed
 * ORGANIC HIERARCHY FORMATION: Works for (byte + byte), (hierarchy + byte), (hierarchy + hierarchy)
 * Pure competition: Strong edges naturally form hierarchies
 * No hardcoded thresholds - emerges from data
 */
static void check_and_form_hierarchy(Graph *graph, Node *from, Node *to, Edge *edge) {
    if (!graph || !from || !to || !edge) return;
    
    // Only form hierarchies if both nodes have payloads (including reference hierarchies)
    if (from->payload_size == 0 || to->payload_size == 0) return;
    
    // Pure competition: Check if this edge is strong enough relative to alternatives
    float local_avg_weight = node_get_local_outgoing_weight_avg(from);
    float epsilon_h = compute_adaptive_epsilon(local_avg_weight);
    
    // Competition: Edge must be stronger than average (data-driven, not threshold)
    float relative_strength = (local_avg_weight > epsilon_h) ? 
        (edge->weight / (local_avg_weight + epsilon_h)) : 1.0f;
    
    // Pure competition: If edge is significantly stronger, form hierarchy
    if (relative_strength > 1.0f) {
        ...
        // Handle reference hierarchies (payload might be NULL, need to reconstruct)
        uint8_t *combined_payload = malloc(combined_size);
        if (combined_payload) {
            size_t offset = 0;
            
            // Get from node's payload (handle reference hierarchies)
            size_t from_size = 0;
            uint8_t *from_payload = node_get_payload(from, &from_size);
            if (from_payload && from_size > 0) {
                size_t copy_size = (from_size < from->payload_size) ? from_size : from->payload_size;
                memcpy(combined_payload + offset, from_payload, copy_size);
                offset += copy_size;
                // Free if it was reconstructed (reference hierarchy)
                if (from->is_reference_hierarchy && !from->payload) {
                    free(from_payload);
                }
            }
            
            // Get to node's payload (handle reference hierarchies)
            size_t to_size = 0;
            uint8_t *to_payload = node_get_payload(to, &to_size);
            if (to_payload && to_size > 0) {
                size_t copy_size = (to_size < to->payload_size) ? to_size : to->payload_size;
                if (offset + copy_size <= combined_size) {
                    memcpy(combined_payload + offset, to_payload, copy_size);
                    offset += copy_size;
                }
                // Free if it was reconstructed (reference hierarchy)
                if (to->is_reference_hierarchy && !to->payload) {
                    free(to_payload);
                }
            }
            
            // Only do trie lookup if we got valid payloads
            if (offset > 0) {
                // O(combined_size) trie lookup with context
                Node *context_arr[2] = { from, to };
                Node *existing = trie_lookup_with_context(graph, combined_payload, offset,
                                                           context_arr, 2, NULL);
        ...
        if (!hierarchy_exists) {
            ...
            Node *hierarchy = create_hierarchy_node(graph, from, to);
            if (hierarchy) {
                ...
                uint8_t *hier_payload = node_get_payload(hierarchy, &hier_size);
                if (hier_payload) {
                    for (size_t k = 0; k < hier_size && k < 20; k++) {
                        if (hier_payload[k] >= 32 && hier_payload[k] < 127) {
                            fprintf(stderr, "%c", hier_payload[k]);
                        } else {
                            fprintf(stderr, "?");
                        }
                    }
                    if (hierarchy->is_reference_hierarchy && !hierarchy->payload) {
                        free(hier_payload);
                    }
                }
                fprintf(stderr, "' (edge weight %.2f, from level %u, to level %u)\n", 
                        edge->weight, from->abstraction_level, to->abstraction_level);
                #endif
            }
        }
    }
                ...
                if (lseek(fd, node_offset, SEEK_SET) == node_offset) {
                    ...
                    if (read(fd, &payload_size, sizeof(size_t)) == sizeof(size_t)) {
                        ...
                        if (node) {
                            ...
                            // CONTEXT: Now uses graph-wide SparseContext (infinite context)
                            // No per-node context_trace storage - skip old format
                            if (props_ok) {
                                // Skip old context_trace format (8 floats + 1 byte)
                                // Read dummy data to maintain file format compatibility
                                float dummy_trace[8];
                                uint8_t dummy_len;
                                read(fd, dummy_trace, sizeof(float) * 8);
                                read(fd, &dummy_len, sizeof(uint8_t));
                                // Read port_id (multi-modal support)
...
int melvin_m_save(MelvinMFile *mfile) {
    ...
    for (size_t i = 0; i < graph->node_count; i++) {
        ...
        // Node data size: payload_size + payload + 6 floats + 1 uint32_t + 2 uint8_t
        // (weight, bias, activation_strength, outgoing_weight_sum, incoming_weight_sum, state) = 6 floats
        // abstraction_level = 1 uint32_t
        // (dummy context_trace for compatibility) = 8 floats + 1 uint8_t
    ...
    for (size_t i = 0; i < graph->node_count; i++) {
        ...
        // CONTEXT: Now uses graph-wide SparseContext (infinite context)
        // Write dummy context_trace for file format compatibility (old format)
        float dummy_trace[8] = {0};
        uint8_t dummy_len = 0;
        if (write(mfile->fd, dummy_trace, sizeof(float) * 8) != sizeof(float) * 8) { free(node_offsets); return -1; }
        if (write(mfile->fd, &dummy_len, sizeof(uint8_t)) != sizeof(uint8_t)) { free(node_offsets); return -1; }
    ...
    if (initial_count > 0) {
        ...
        for (size_t i = 0; i < initial_count && initial_nodes; i++) {
            ...
            if (i < initial_count) {
                ...
                // CONTEXT: Now uses graph-wide SparseContext (infinite context)
                // No per-node context_trace storage - context is the ActivationPattern
                // Sequence position information is maintained in ActivationPattern
        ...
        for (size_t i = 0; i + 1 < initial_count && initial_nodes; i++) {
            ...
            if (edge) {
                ...
                // PHASE 1: Process input sequence (creates 2-byte hierarchies: byte + byte)
                // PRINCIPLE 2: No hardcoded thresholds - use pure competition
                // PRINCIPLE 4: Compounding learning - hierarchies form naturally from strong patterns
                ...
                // === CREATE SIMILARITY EDGES (Brain: Similar patterns connect) ===
                // Create edges to similar nodes for generalization
                // Only for new nodes (check if node has few edges)
                if (to->outgoing_count < 5) {
                    graph_create_similarity_edges_for_node(mfile->graph, to, NULL);
                }
                
                // === CREATE HOMEOSTATIC EDGES (Brain: Maintain minimum connectivity) ===
                // Ensure nodes have minimum connections
                graph_create_homeostatic_edges_for_node(mfile->graph, to);
            }  // End if (i < initial_count && (i + 1) < initial_count)
        }
        
        // === CREATE CONTEXT EDGES (Brain: Co-activation creates connections) ===
        // After processing all sequential edges, create context edges from activation pattern
        if (pattern && pattern->count > 1) {
            graph_create_context_edges_from_pattern(mfile->graph, pattern, NULL);
        }
        
        // === PHASE 2: ORGANIC MULTI-LEVEL HIERARCHY FORMATION ===
        // Process ALL activated nodes (including hierarchies!) to form larger hierarchies
        // This enables: (hierarchy + byte) → 3+ byte hierarchies
        //              (hierarchy + hierarchy) → even larger hierarchies
        // ORGANIC: Hierarchies naturally combine as patterns repeat
        // COMPUTE SAVINGS: Larger hierarchies = fewer node traversals = faster generation
        if (pattern && pattern->nodes && pattern->count > 1) {
            // Process edges between ALL activated nodes (not just input sequence)
            for (size_t i = 0; i < pattern->count; i++) {
                Node *from = pattern->nodes[i];
                if (!from || from->payload_size == 0) continue;
                ...
                for (size_t j = 0; j < from->outgoing_count; j++) {
                    ...
                    // Only form hierarchy if 'to' is also in activation pattern (co-activation)
                    // This ensures hierarchies form from patterns that actually occur together
                    int to_is_activated = 0;
                    for (size_t k = 0; k < pattern->count; k++) {
                        if (pattern->nodes[k] == to) {
                            to_is_activated = 1;
                            break;
                        }
                    }
                    ...
                    if (to_is_activated && to->payload_size > 0) {
                        // Check if this edge should form a hierarchy
...
static void melvin_generate_output_from_state(MelvinMFile *mfile, WaveState *state, 
    ...
    if (candidate_capacity > 0) {
        ...
        // BRAIN-LIKE RELATIVE THRESHOLD: Neurons fire relative to strongest signal
        // Principle 1: "All decisions are relative to local context"
        // Find max activation first (strongest signal in this context)
        ...
        // High variance = stricter threshold (more selective)
        // Low variance = more permissive (include more candidates)
        float activation_variance = 0.0f;
        float activation_mean = 0.0f;
        if (state->all_activated_count > 0 && max_activation > 0.0f) {
            // Compute mean
            for (size_t i = 0; i < state->all_activated_count; i++) {
                activation_mean += state->all_activation_strengths[i];
            }
            activation_mean /= (float)state->all_activated_count;
            
            // Compute variance
            for (size_t i = 0; i < state->all_activated_count; i++) {
                float diff = state->all_activation_strengths[i] - activation_mean;
                activation_variance += diff * diff;
            }
            activation_variance /= (float)state->all_activated_count;
        }
        
        // Relative threshold: adapts to variance (data-driven)
        // High variance → stricter (only strongest nodes)
        // Low variance → more permissive (include more nodes)
        ...
        relative_threshold = (relative_threshold > 0.5f) ? 0.5f : relative_threshold;  // Cap at 0.5
        
        // Filter wave-activated nodes: only include nodes that "fired" relative to strongest
        // Brain-like: Neurons fire relative to local context, not absolute threshold
        for (size_t i = 0; i < state->all_activated_count; i++) {
            ...
            // RELATIVE THRESHOLD: Compare activation to strongest signal
            // If max_activation is 0, include all (no signal to compare against)
            ...
            // Add to candidates (node fired relative to strongest - it's in the active set)
    ...
    for (size_t i = 0; i < candidate_count; i++) {
        ...
        // CRITICAL FIX: Use context-aware edge weights for accurate predictions
        // Context = input + output nodes so far (matches training context)
        if (input_nodes && input_count > 0) {
            // Build SparseContext from input + output nodes for context-aware edge selection
            SparseContext *prediction_ctx = NULL;
            size_t total_ctx_count = input_count + (state ? state->all_activated_count : 0);
            if (total_ctx_count > 0) {
                Node **ctx_nodes = malloc(total_ctx_count * sizeof(Node*));
                float *ctx_activations = malloc(total_ctx_count * sizeof(float));
                ...
                if (ctx_nodes && ctx_activations) {
                    ...
                    // Add activated nodes from wave propagation (output context)
                    if (state && state->all_activated_nodes) {
                        for (size_t k = 0; k < state->all_activated_count && idx < total_ctx_count; k++) {
                            ctx_nodes[idx] = state->all_activated_nodes[k];
                            ctx_activations[idx] = state->all_activation_strengths[k];
                            idx++;
                        }
                    }
                    if (idx > 0) {
                        prediction_ctx = sparse_context_create_from_nodes(ctx_nodes, ctx_activations, idx);
                    }
                    free(ctx_nodes);
                    free(ctx_activations);
                }
            ...
            // Use context-aware edge weights for accurate predictions
            for (int input_idx = (int)input_count - 1; input_idx >= 0; input_idx--) {
                ...
                for (size_t j = 0; j < input_node->outgoing_count; j++) {
                    ...
                    if (edge && edge->to_node == node) {
                        // === ORGANIC APPROACH: Use raw signals, no hardcoded formulas ===
                        // Get context match quality (raw signal)
                        float context_match_quality = edge_compute_context_weight(edge, prediction_ctx);
                        
                        // Get edge weight relative to local avg (raw signal)
                        float base_weight = (float)edge->weight / 255.0f;
                        float local_avg = node_get_local_outgoing_weight_avg(input_node);
                        float epsilon = compute_adaptive_epsilon(local_avg);
                        float base_relative = (local_avg > epsilon) ? (base_weight / local_avg) : base_weight;
                        
                        // Add both signals to logit - let softmax decide which wins
                        // No hardcoded formula - both signals contribute, softmax picks winner
                        logits[i] += (context_match_quality + base_relative) * base_logit;
                        break;  // Found match, no need to check other edges from this node
                    }
                }
            }
            
            sparse_context_free(prediction_ctx);
        }
    ...
    size_t max_iterations = 10000;  // Safety limit to prevent infinite loops
    size_t iteration = 0;
    ...
    while (1) {
        ...
        // ORGANIC: No separate stop checks - stopping competes with edges in node_compute_winning_edge_with_context
        // If stop_weight wins, the function returns NULL and we break naturally below
        ...
        if (output_len == 0 && candidate_count > 0) {
            ...
            if (last_output_node && last_output_node->outgoing_count > 0) {
                ...
                if (chosen_edge && chosen_edge->to_node && chosen_edge->to_node->payload_size > 0) {
                    ...
                    // No valid edge (or stop won competition) - natural stopping point
                    /* stop_reason=natural_stop */
                    break;
                }
            } else {
                // Node has no outgoing edges - natural stopping point
                /* stop_reason=natural_stop */
                break;
            }
        } else {
        ...
        // Output bytes from node (hierarchy nodes output all bytes, raw nodes output one byte)
        // Following README: "use compressed knowledge" - hierarchies output full payload
        size_t bytes_to_output = 1;  // Default: single byte (for raw nodes)
        if (sampled_node && sampled_node->abstraction_level > 0) {
            // Hierarchy node: output ALL bytes (compressed knowledge)
            bytes_to_output = sampled_node->payload_size;
        }
        
        // Output all bytes from node's payload
        int eos_detected = 0;
        for (size_t b = 0; b < bytes_to_output; b++) {
            if (b >= sampled_node->payload_size) break;
            uint8_t byte = sampled_node->payload[b];
            if (byte == EOS_BYTE) {
                /* stop_reason=EOS */
                eos_detected = 1;
                break;
            ...
            if (output_len >= output_capacity) {
                size_t new_capacity = (output_capacity == 0) ? 16 : output_capacity * 2;
            ...
            // Track the node for each byte position (works with all abstraction levels)
            // Grow output_nodes array if needed
            if (output_len > output_nodes_capacity) {
                size_t new_capacity = (output_nodes_capacity == 0) ? 16 : output_nodes_capacity * 2;
            ...
            if (output_nodes && output_len - 1 < output_nodes_capacity) {
                output_nodes[output_len - 1] = sampled_node;  // Store node for this byte position
            }
        }
        
        // Stop if EOS was detected
        if (eos_detected) {
            break;
        }
        ...
        // CONTEXT: Uses graph-wide SparseContext (infinite context)
        // Context = all activated nodes from wave propagation (no window needed)
        // Sequence position information maintained in ActivationPattern
        // No per-node context_trace updates needed - context is already infinite
        ...
        // ORGANIC: No separate stop checks - stopping competes with edges locally
        // If a node has no outgoing edges, node_compute_winning_edge_with_context returns NULL
        // and we break naturally above. Stop decisions are local, data-driven.
    ...
    // NO FALLBACK: If no output generated, output remains empty
    // Requirement.md line 5: "No Fallbacks" - system must learn to generate output organically
